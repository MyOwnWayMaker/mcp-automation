...
const TOOLS: Tool[] = [
  ...
  { name: "notarygadget_complete_signing", description: "Record notarial acts for a signing (use notarization_count=0 if none)", inputSchema: { type: "object", properties: { signing_id: { type: "string", description: "Signing ID (leave blank for most recent)" }, notarization_count: { type: "number", description: "Number of notarial acts performed (0 = none)" }, date: { type: "string", description: "YYYY-MM-DD date of signing (optional, defaults to signing date)" } }, required: ["notarization_count"] } },
