...
import { filetracListCompanies, filetracListClaims, filetracGetClaim, filetracUpdateClaimDates, filetracAddNote, filetracSubmitTimeExpense, filetracGetNotes, filetracBulkGetClaims, filetracBulkAddNote, filetracListDocuments, filetracDownloadReport } from "./tools/filetrac.js";
...
const TOOLS: Tool[] = [
  ...
  { name: "drive_upload_file", description: "Upload a local file (PDF, MP4, ZIP, image, etc.) to Google Drive. Infers MIME type from extension automatically.", inputSchema: { type: "object", properties: { local_path: { type: "string", description: "Absolute path to the local file to upload" }, folder_id: { type: "string", description: "Drive folder ID to upload into (optional)" }, name: { type: "string", description: "Override filename on Drive (defaults to original filename)" }, mime_type: { type: "string", description: "Override MIME type (optional, auto-detected from extension)" } }, required: ["local_path"] } },
  ...
  // XactAnalysis
  { name: "xact_list_assignments", description: "List XactAnalysis assignments. Use include_all=true to see all assignments regardless of date, or since_date (YYYY-MM-DD) to filter from a specific date. Defaults to 2-year window.", inputSchema: { type: "object", properties: { status: { type: "string", enum: ["in_progress", "returned", "all"] }, max_results: { type: "number" }, since_date: { type: "string", description: "YYYY-MM-DD — only show assignments received on or after this date" }, include_all: { type: "boolean", description: "Remove date restriction entirely and return all assignments" } }, required: [] } },
  ...
  { name: "filetrac_list_documents", description: "List all uploaded documents/reports for a FileTrac claim. Returns report_id, filename, date, file_type, size, URL, and description for each document. Use report_id or URL with filetrac_download_report to download.", inputSchema: { type: "object", properties: { claim_id: { type: "string", description: "FileTrac claim ID (numeric, from claimID= in URL)" }, company_index: { type: "number", description: "0=Accelerated, 1=Premier Claims (default), 2=Stewardship, 3=US Claim Solutions" } }, required: ["claim_id"] } },
  { name: "filetrac_download_report", description: "Download an uploaded FileTrac report/document to a local file path. Provide either report_url (from filetrac_list_documents) or both claim_id + report_id to look it up automatically.", inputSchema: { type: "object", properties: { claim_id: { type: "string", description: "FileTrac claim ID (required if report_url not provided)" }, report_id: { type: "string", description: "Report ID from filetrac_list_documents (required if report_url not provided)" }, report_url: { type: "string", description: "Direct data-path URL from filetrac_list_documents (use this instead of claim_id+report_id for speed)" }, dest_path: { type: "string", description: "Local file path to save the download (e.g. ~/Desktop/claim_12345.pdf)" }, company_index: { type: "number" } }, required: ["dest_path"] } },
