...
const TOOLS: Tool[] = [
  ...
  // NotaryGadget
  { name: "notarygadget_create_signing", description: "Create a new signing order in NotaryGadget", inputSchema: { type: "object", properties: { customer: { type: "string", description: "Company name (e.g. Pickford Escrow)" }, date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:MM (24h or 12h)" }, fee: { type: "number", description: "Fee amount (e.g. 150, 250, 75)" }, location: { type: "string", description: "Street address (e.g. '4328 Ben Ave') or full address including city/state" }, city: { type: "string", description: "City (optional if included in location)" }, state: { type: "string", description: "2-letter state code, default CA" }, zip: { type: "string", description: "Zip code (optional)" }, signer_names: { type: "array", items: { type: "string" } }, package_type: { type: "string", description: "e.g. Seller's package, Buyer's package, Single document" }, notes: { type: "string" } }, required: ["customer", "date", "time", "fee", "location", "signer_names"] } },
  { name: "notarygadget_complete_signing", description: "Record notarial acts for a signing. Defaults to 'no notarial acts' — only pass notarization_count if acts were actually performed.", inputSchema: { type: "object", properties: { signing_id: { type: "string", description: "Signing ID (leave blank for most recent)" }, notarization_count: { type: "number", description: "Number of notarial acts performed — omit or set to 0 to record none" }, date: { type: "string", description: "YYYY-MM-DD (optional)" } }, required: [] } },
