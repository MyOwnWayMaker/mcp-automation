...
const TOOLS: Tool[] = [
  ...
  { name: "pick_inspection_slots", description: "Step 5 of the post-claim playbook — given a loss address, geocode it, classify its quadrant, scan the calendar over the next N days (default 3), and return up to max_slots (default 5) candidate 1-hour inspection windows ranked by preference. If any timed event is in the same quadrant, slots adjacent to that event come first; otherwise the earliest free windows within working hours (default 7AM–4PM LA local). All-day events block their entire day. Each slot is tagged with the closest prev/next event-with-location so C4 drive-time validation can run without re-scanning the calendar.", inputSchema: { type: "object", properties: { loss_address: { type: "string" }, days: { type: "number", description: "Days to look ahead (default 3)" }, time_min: { type: "string", description: "Override 'now' (ISO). Default: now." }, calendar_id: { type: "string", description: "Default 'primary'" }, max_slots: { type: "number", description: "Default 5" }, work_start_hour: { type: "number", description: "LA-local working window start hour (default 7)" }, work_end_hour: { type: "number", description: "LA-local working window end hour (default 16)" }, slot_minutes: { type: "number", description: "Inspection window length in minutes (default 60)" } }, required: ["loss_address"] } },
];
