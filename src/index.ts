...
const TOOLS: Tool[] = [
  ...
  // NotaryGadget
  { name: "notarygadget_create_signing", description: "Create a new signing order in NotaryGadget. ZIP code is required — include it in location (e.g. '4328 Ben Ave, Studio City, CA 91604') or pass it as the 'zip' parameter.", inputSchema: { type: "object", properties: { customer: { type: "string", description: "Company name (e.g. Pickford Escrow)" }, date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:MM (24h or 12h)" }, fee: { type: "number", description: "Fee amount (e.g. 150, 250, 75)" }, location: { type: "string", description: "Full address including ZIP (e.g. '4328 Ben Ave, Studio City, CA 91604')" }, city: { type: "string", description: "City (optional if included in location)" }, state: { type: "string", description: "2-letter state code, default CA" }, zip: { type: "string", description: "5-digit ZIP code — REQUIRED if not included in location string" }, signer_names: { type: "array", items: { type: "string" } }, package_type: { type: "string", description: "e.g. Seller's package, Buyer's package, Single document" }, notes: { type: "string" } }, required: ["customer", "date", "time", "fee", "location", "signer_names"] } },
