...
import { gmailListScheduledSends, gmailCancelScheduledSend, gmailRunScheduledSweep } from "./tools/scheduled_send.js";
import { gmailEmailToPdf } from "./tools/gmail_email_to_pdf.js";
...
import { fetchClaimDetailsTool } from "./tools/claim_fetcher.js";
import { scanCalendarWithQuadrantsTool } from "./tools/calendar_scan.js";
import { pickInspectionSlotsTool } from "./tools/slot_picker.js";
import { draftInspectionSmsTool } from "./tools/sms_drafter.js";
import { sendApprovalPromptTool } from "./tools/approval_prompt.js";
import { handleClaimApprovalTool } from "./tools/approval_handler.js";
import { checkFollowupDueTool } from "./tools/followup_check.js";
import { createClaimDriveFolderTool } from "./tools/claim_drive_folder.js";
...
import { notarygadgetCreateSigning, notarygadgetUpdateSigning, notarygadgetCompleteSigning, notarygadgetEnterMileage, notarygadgetRecordPayment, notarygadgetUpdatePayment, notarygadgetDeletePayment, notarygadgetGetSignings, notarygadgetGetPayments, notarygadgetSendInvoice, notarygadgetDeleteSigning, notarygadgetUpdateCustomer } from "./tools/notarygadget.js";
...
import { xactListAssignments, xactGetAssignment, xactUpdateDates, xactUpdateWorkflowStatus, xactAddNote, xactGetNotes, xactDeleteNote, xactSetPlannedInspectionDate, xactFindAssignmentByClaim, xactFindAssignmentByName, xactListDocuments, xactDownloadDocument } from "./tools/xactanalysis.js";
...
const TOOLS: Tool[] = [
  ...
  { name: "gmail_send_draft", description: "Send an existing Gmail draft and remove it from the Drafts folder in one API call. Reads the draft back BEFORE sending so the returned response carries the exact snapshot that was sent. Use this when a draft has been reviewed and is ready to ship — sending via gmail_send_email would orphan the draft. STRICT-SEND GUARDRAIL: the draft's stored recipients (To/Cc/Bcc) are inspected; if ANY is on a third-party domain (outside GMAIL_INTERNAL_DOMAINS — default erseville.com), the tool REFUSES unless approved_at_iso_timestamp is within the last 15 min AND force_send=true. Internal-only drafts bypass. This blocks blind auto-sends of drafts the watchers created — the contract is 'create draft → Hakiel reviews in Gmail → Hakiel sends manually'; automated callers must NOT shortcut this.", inputSchema: { type: "object", properties: { draft_id: { type: "string", description: "Numeric Gmail draft ID (from gmail_create_draft output or Gmail compose URL ?compose=<id>)" }, approved_at_iso_timestamp: { type: "string", description: "ISO-8601 timestamp of explicit Hakiel approval to ship this draft to a third-party recipient. Must be within the last 15 minutes. Pair with force_send:true. Omit for internal-only drafts." }, force_send: { type: "boolean", description: "Set true alongside a fresh approved_at_iso_timestamp to bypass the strict-send guardrail for a one-off third-party draft send." } }, required: ["draft_id"] } },
  { name: "gmail_create_draft_scheduled", description: "Create a Gmail draft and AUTO-SEND it at send_at (ISO-8601). Gmail's native Schedule-Send isn't API-exposed, so this emulates it with two complementary firing paths: an IN-PROCESS TIMER (fires on time while the MCP server is running, re-armed on boot from a Gmail label) AND a LOCAL LAUNCHD SWEEP at ~5-min cadence (backstop if the server was down at send_at). Both paths are idempotent — whichever fires first sends the draft; the other gets a no-op 404. STRICT-SEND GUARDRAIL applies because it auto-fires: third-party recipients require approved_at_iso_timestamp within 15 min AND force_send=true; internal-only recipients bypass. The sweep also re-checks recipients at maturity — if it's a third-party draft it does NOT auto-send, it surfaces the draft to ntfy for manual review instead. CO-LOCATION: this tool MUST be served by the Mac-mini local MCP server; if served by Railway, the queue write lands on ephemeral container disk and the sweep never sees it (silent never-send). Cancel before send_at with gmail_delete_draft (stops the timer AND lets the sweep drop the entry on the next tick) — or use gmail_cancel_scheduled_send to remove only the queue entry without deleting the draft.", inputSchema: { type: "object", properties: { to: { type: "string" }, subject: { type: "string" }, body: { type: "string" }, send_at: { type: "string", description: "ISO-8601 datetime to send at. Must be in the future and within 1 year." }, cc: { type: "string" }, bcc: { type: "string" }, approved_at_iso_timestamp: { type: "string", description: "ISO-8601 approval timestamp (within last 15 min) for a third-party scheduled send. Pair with force_send:true." }, force_send: { type: "boolean", description: "Set true with a fresh approved_at_iso_timestamp to authorize a third-party scheduled send." } }, required: ["to", "subject", "body", "send_at"] } },
  { name: "gmail_list_scheduled_sends", description: "List every entry currently in the local scheduled-send queue (scheduled_sends.json). Shows status (⏳ pending in N min / ⌛ due / 🔸 surfaced for manual send / ⚠️ bad date), auto-vs-manual policy, subject, recipient, last error if any, and draft ID. Read-only — does not fire any sends. Useful for sanity-checking what's queued before/after a launchd sweep.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "gmail_cancel_scheduled_send", description: "Remove a scheduled-send queue entry by draft_id WITHOUT deleting the underlying Gmail draft. Use gmail_delete_draft if you also want to discard the composed message (gmail_delete_draft will additionally stop the in-process timer; this tool only cleans the queue). Returns a no-op message if no entry matches.", inputSchema: { type: "object", properties: { draft_id: { type: "string", description: "Gmail draft ID of the queued scheduled send to cancel." } }, required: ["draft_id"] } },
  { name: "gmail_run_scheduled_sweep", description: "Manually trigger the scheduled-send sweep right now (instead of waiting for the launchd cron tick). Processes every matured entry in the queue: internal-only drafts auto-send via drafts.send, third-party drafts get surfaced to ntfy for manual review (never auto-sent). Returns a structured summary (checked/matured/sent/surfaced/dropped/errors/kept). Use for debugging or flushing a pending send without waiting up to 5 minutes for the cron.", inputSchema: { type: "object", properties: {}, required: [] } },
  ...
  { name: "gmail_email_to_pdf", description: "Render a Gmail message as a Letter-sized PDF (mirrors Gmail's Print → Save as PDF). Pass message_id (required); optionally drive_folder_id to upload directly + filename to override the subject-based default (replaces `:` `|` `/` `\\` with `_`). Returns the base64 PDF when no folder is set, or a Drive upload confirmation when it is. Used by the claim-folder pipeline to snapshot assignment emails, DA notes, mileage approvals, etc.", inputSchema: { type: "object", properties: { message_id: { type: "string", description: "Gmail message ID" }, drive_folder_id: { type: "string", description: "Drive folder to upload the rendered PDF into (optional — without this the PDF is returned as base64)" }, filename: { type: "string", description: "Override the auto-generated subject-based filename. Use for content-based labels like 'Approved Mileage.pdf'." } }, required: ["message_id"] } },
  ...
  { name: "extract_pdf_text", description: "Extract text from a PDF. Server-side — no client sandbox required. Pass exactly one of: file_path, drive_file_id, or gmail_message_id + attachment_id. Returns extracted text + page count + a `Source: text_layer|ocr|empty` line. For PDFs without a text layer (scanned images), automatically falls back to OCR via tesseract + poppler. OCR responses include per-page confidence so callers can warn before relying on critical fields (phone numbers, name spellings).", inputSchema: { type: "object", properties: { file_path: { type: "string" }, drive_file_id: { type: "string" }, gmail_message_id: { type: "string" }, attachment_id: { type: "string" }, page_range: { type: "string", description: "Reserved for per-page extraction; currently informational only — full document is returned." } } } },
  { name: "gmail_attachment_text", description: "One-call shortcut: given a Gmail message ID and a PDF attachment filename (exact match or case-insensitive substring), download the PDF and return its text. Combines gmail_download_attachment + extract_pdf_text. Falls back to OCR automatically for scanned PDFs.", inputSchema: { type: "object", properties: { message_id: { type: "string" }, attachment_filename: { type: "string", description: "Exact filename or case-insensitive substring (e.g. 'LossNotice')" } }, required: ["message_id", "attachment_filename"] } },
  ...
  { name: "notion_add_database_property", description: "Add a new property (column) to a Notion database. Supports: number, text, checkbox, date, url, email, phone_number, select, multi_select, formula. For number columns, optionally specify number_format (e.g. 'number', 'dollar', 'percent'). For formula columns, pass formula_expression with the Notion formula text (e.g. 'prop(\"A\") + prop(\"B\")').", inputSchema: { type: "object", properties: { database_id: { type: "string" }, property_name: { type: "string" }, property_type: { type: "string", enum: ["number", "text", "checkbox", "date", "url", "email", "phone_number", "select", "multi_select", "formula"] }, number_format: { type: "string", description: "Only for number type. Options: number, number_with_commas, percent, dollar, euro, pound, yen, etc." }, formula_expression: { type: "string", description: "Required for formula type. Notion formula expression text. Empty number cells are treated as 0 in arithmetic." } }, required: ["database_id", "property_name", "property_type"] } },
  ...
  { name: "notion_archive_page", description: "Archive (soft-delete) a Notion page or database item by page_id. Recoverable from Notion trash for 30 days. Use this for database rows — notion_archive_block does NOT work on database items because Notion treats them as pages. Calls PATCH /v1/pages/:page_id with archived=true.", inputSchema: { type: "object", properties: { page_id: { type: "string" } }, required: ["page_id"] } },
  { name: "notion_restore_page", description: "Restore an archived Notion page or database item. Symmetric to notion_archive_page — calls PATCH /v1/pages/:page_id with archived=false.", inputSchema: { type: "object", properties: { page_id: { type: "string" } }, required: ["page_id"] } },
  { name: "notion_reorder_properties", description: "Reorder columns in a Notion database. Pass property_order as the desired left-to-right list of property names. Unlisted properties keep their relative order at the end. Errors if any name is misspelled (won't silently drop). Notion's default-view column order generally follows schema order but a UI refresh may be needed to reflect the change.", inputSchema: { type: "object", properties: { database_id: { type: "string" }, property_order: { type: "array", items: { type: "string" }, description: "Property names in the desired left-to-right order" } }, required: ["database_id", "property_order"] } },
  ...
  { name: "notary_send_email", description: "Send an email from drupenterprise1@gmail.com. STRICT-SEND GUARDRAIL: if the recipient is on a third-party domain (outside GMAIL_INTERNAL_DOMAINS — default erseville.com), the tool REFUSES unless approved_at_iso_timestamp is within the last 15 min AND force_send=true. Internal-only sends bypass. This blocks blind auto-replies on the notary inbox — drafts go through pickford_drafter for Hakiel's manual review.", inputSchema: { type: "object", properties: { to: { type: "string" }, subject: { type: "string" }, body: { type: "string" }, reply_to_message_id: { type: "string" }, thread_id: { type: "string" }, approved_at_iso_timestamp: { type: "string", description: "ISO-8601 timestamp of explicit Hakiel approval for a third-party notary send. Must be within the last 15 minutes. Pair with force_send:true." }, force_send: { type: "boolean", description: "Set true alongside a fresh approved_at_iso_timestamp to bypass the strict-send guardrail." } }, required: ["to", "subject", "body"] } },
  ...
  { name: "gmail_notary_reply_to_email", description: "Reply to an email thread from drupenterprise1@gmail.com. STRICT-SEND GUARDRAIL: the recipient (original message's From header) is inspected; if on a third-party domain (outside GMAIL_INTERNAL_DOMAINS — default erseville.com), the tool REFUSES unless approved_at_iso_timestamp is within the last 15 min AND force_send=true. This blocks blind auto-replies on the notary inbox.", inputSchema: { type: "object", properties: { message_id: { type: "string" }, body: { type: "string" }, approved_at_iso_timestamp: { type: "string", description: "ISO-8601 timestamp of explicit Hakiel approval for a third-party reply. Must be within the last 15 minutes. Pair with force_send:true." }, force_send: { type: "boolean", description: "Set true alongside a fresh approved_at_iso_timestamp to bypass the strict-send guardrail." } }, required: ["message_id", "body"] } },
  ...
  { name: "xact_get_assignment", description: "Get full detail for a XactAnalysis assignment by MFN code. Optional `tab` param: 'details' (default), 'client_policy' (returns structured JSON: loss_address, mailing_address, insured, policy, coverage), 'notes', 'documents', 'map', 'action_items', 'history'.", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" }, tab: { type: "string", enum: ["details", "client_policy", "notes", "documents", "map", "action_items", "history"], description: "Which tab's content to return. Defaults to 'details'." } }, required: ["mfn"] } },
  { name: "xact_get_client_policy", description: "Returns the CLIENT/POLICY tab of an XA assignment as structured JSON: loss_address, mailing_address, insured, policy, coverage. Convenience wrapper around xact_get_assignment with tab=client_policy — surfaces the dedicated arg through the deferred-tool connector which would otherwise strip the tab parameter.", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" } }, required: ["mfn"] } },
  { name: "xact_get_documents", description: "List the documents on an XA assignment as structured JSON: [{filename, image_id, date}]. The image_id is the handle for xact_download_document. (Clicks the real d_imgprint Documents tab.)", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" } }, required: ["mfn"] } },
  { name: "xact_download_document", description: "Download one document's bytes from an XA assignment by image_id (from xact_get_documents). Returns base64 ready to pass to drive_upload_file(file_bytes_b64). Fetches CXAimgoutput.jsp with the live session.", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code (e.g. 06QRD2C)" }, image_id: { type: "string", description: "Document image_id from xact_get_documents" }, filename: { type: "string", description: "Optional filename to label the result" } }, required: ["mfn", "image_id"] } },
  { name: "xact_get_assignment_notes", description: "Returns the NOTES tab of an XA assignment — examiner/admin notes timeline. Convenience wrapper around xact_get_assignment with tab=notes. (Distinct from xact_get_notes which lists *adjuster* notes.)", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" } }, required: ["mfn"] } },
  { name: "xact_get_history", description: "Returns the HISTORY tab of an XA assignment — full audit log of status changes. Convenience wrapper around xact_get_assignment with tab=history.", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" } }, required: ["mfn"] } },
  ...
  { name: "xact_delete_note", description: "Delete a note from a XactAnalysis assignment. Pass either note_id (XA's internal ID) or note_text_match (substring of the note body — first match wins). Best-effort: returns a debug snapshot if the delete control can't be located.", inputSchema: { type: "object", properties: { mfn: { type: "string" }, note_id: { type: "string", description: "XA's internal note ID (if known)" }, note_text_match: { type: "string", description: "Substring of the note body to match — first matching note will be deleted" } }, required: ["mfn"] } },
  { name: "xact_set_planned_inspection_date", description: "Set the Planned Inspection Date on an XA assignment via the workflow-status iframe dialog. Pass dry_run: true to capture full form state (every field + value) and dismiss the dialog without submitting — useful for diagnosing which hidden field holds the canonical date when the visible input override doesn't take.", inputSchema: { type: "object", properties: { mfn: { type: "string" }, date: { type: "string", description: "YYYY-MM-DD or M/D/YYYY" }, dry_run: { type: "boolean", description: "Capture form state + dismiss without submit (no XA writes)" } }, required: ["mfn", "date"] } },
  ...
  { name: "filetrac_add_note", description: "Add a diary note to a FileTrac claim with read-write-read verification. Provide claim_id (preferred — enables verification) or file_number. Pre-checks for duplicates, posts the note, then re-reads to confirm it landed (FileTrac returns 500 on successful commits, so HTTP status is unreliable). Notes are PERMANENT and visible to clients.", inputSchema: { type: "object", properties: { file_number: { type: "string", description: "8-digit FileTrac file number (e.g. 81030471). If only this is given, verification is skipped." }, claim_id: { type: "string", description: "Numeric claim ID — REQUIRED for read-write-read verification. File number will be looked up automatically." }, note: { type: "string" }, category: { type: "string", description: "Note category. Premier Claims options: None, Contractor Estimate Received, EMS Estimate Received, Inspection Needed, Prior Payment, Revisions Needed, Supplement Request" }, visible_to_client: { type: "boolean" }, company_index: { type: "number" }, dry_run: { type: "boolean", description: "If true, GET the form and report what WOULD be POSTed (incl. valid category options) without actually submitting" }, force: { type: "boolean", description: "If true, post even when a duplicate of this note text is already in the diary. Default false." }, skip_verify: { type: "boolean", description: "If true, skip the read-write-read sandwich (fire-and-forget). Default false. Only set this for batch operations where verification is handled separately." } }, required: ["note"] } },
  ...
  { name: "voice_list_threads", description: "Google Voice: list recent SMS/MMS threads from the inbox with contact, preview, last activity, and unread flag. Replaces Chrome-MCP scraping which takes 200+ tool turns. Auth: voice_session.json (run scripts/auth-voice.mjs once).", inputSchema: { type: "object", properties: { limit: { type: "number", description: "Max threads (default 20)" }, since: { type: "string", description: "ISO 8601 — only return threads with last activity at/after this" } }, required: [] } },
  { name: "voice_get_thread", description: "Google Voice: return EVERY message in a thread with sender, timestamp, body, direction, and media flags. Provide thread_id (preferred) or contact (name or phone). Scrolls to thread start by default to load full history. Returns structured JSON, ISO 8601 timestamps where parseable.", inputSchema: { type: "object", properties: { thread_id: { type: "string", description: "Voice thread ID (from /messages/t/<id> URL)" }, contact: { type: "string", description: "Contact name or phone number — used when thread_id is unknown" }, scroll_to_start: { type: "boolean", description: "Scroll to load full history (default true)" }, order: { type: "string", enum: ["oldest_first", "newest_first"], description: "Default oldest_first" }, max_messages: { type: "number", description: "Cap (default 5000)" } }, required: [] } },
  { name: "voice_dump_html", description: "Google Voice diagnostic: fetch any voice.google.com URL with the saved session and return rendered HTML, optionally only chunks around a search string. Mirror of filetrac_dump_html — used to inspect markup when a parser misses content.", inputSchema: { type: "object", properties: { path: { type: "string", description: "Relative path on voice.google.com or full URL (default /u/0/messages)" }, search: { type: "string", description: "Optional substring; only chunks around matches returned" }, context_chars: { type: "number", description: "Chars of context around each match (default 1500)" }, max_matches: { type: "number", description: "Cap on matches returned (default 10)" }, scroll_to_start: { type: "boolean", description: "Scroll the message pane to the top before dumping (default false)" }, wait_ms: { type: "number", description: "Extra ms to wait after navigation (default 3000). Voice loads thread data via long-poll XHR — bump to 10000+ to capture post-XHR DOM." }, wait_for_selector: { type: "string", description: "If set, wait up to 30s for this CSS selector to appear before dumping. e.g. 'a[href*=\\\"/messages/t/\\\"]' for thread anchors." } }, required: [] } },
  { name: "voice_search_messages", description: "Google Voice full-text search across threads. STUB — pending Dispatch's RPC probe (returns a 'not yet implemented' message).", inputSchema: { type: "object", properties: { query: { type: "string" }, since: { type: "string" }, until: { type: "string" } }, required: ["query"] } },
  { name: "voice_get_voicemails", description: "Google Voice voicemails with caller, timestamp, transcript, audio URL, and read flag. STUB — pending Dispatch's RPC probe.", inputSchema: { type: "object", properties: { limit: { type: "number" }, since: { type: "string" } }, required: [] } },
  { name: "voice_send_sms", description: "Send an SMS via Google Voice and verify it landed in the thread. Provide thread_id (existing thread) OR number (new conversation). Pre-reads thread, types body into compose, clicks Send, then polls for up to 12s for the new message to appear. Returns { ok, verified, message, url } on success or { ok: false, error, debug } with diagnostic info on failure. May be blocked by Voice's bot detection on Railway — use scripts/test-voice-send.mjs to run locally on the VM if needed.", inputSchema: { type: "object", properties: { thread_id: { type: "string", description: "Existing thread (e.g. '+15551234567' or 't.+15551234567')" }, number: { type: "string", description: "Phone number for a new conversation" }, body: { type: "string" }, force: { type: "boolean", description: "Skip duplicate-suppression pre-check (reserved)" }, skip_verify: { type: "boolean", description: "Skip post-send verification poll" } }, required: ["body"] } },
  ...
  { name: "notarygadget_update_customer", description: "Update a NotaryGadget customer (Contact) record — company name, billing/office address, primary contact, phone, email, invoice email, instructions, notes. Pass either customer_id (preferred, exact) or customer_name (must match exactly one). Only the provided fields are changed; everything else stays. Re-reads after save and reports a per-field before→after diff, flagging any field that didn't persist.", inputSchema: { type: "object", properties: { customer_id: { type: "string", description: "Numeric NG customer (contact) ID — preferred when known" }, customer_name: { type: "string", description: "Search term to resolve a single customer; fails if it matches more than one" }, address1: { type: "string", description: "Street address line 1" }, address2: { type: "string", description: "Suite/Unit (address line 2)" }, city: { type: "string" }, state: { type: "string", description: "2-letter state code" }, zip: { type: "string" }, company: { type: "string", description: "Company name (rename)" }, first: { type: "string", description: "Primary contact first name" }, last: { type: "string", description: "Primary contact last name" }, title: { type: "string" }, email: { type: "string" }, office: { type: "string", description: "Office phone" }, cell: { type: "string", description: "Cell phone" }, fax: { type: "string" }, website: { type: "string" }, invoice_email: { type: "string", description: "Email(s) used for invoices — comma-separate multiple" }, notes: { type: "string" }, instructions: { type: "string", description: "Special instructions" } }, required: [] } },
  ...
  { name: "fetch_claim_details", description: "Given platform + claim_number, dispatch to the right backend (filetrac_get_claim or xact_get_assignment) and return a unified {claim_number, carrier, insured_name, insured_phone, loss_address, mailing_address, date_of_loss, loss_type, policy_number, date_of_first_contact?, date_of_inspection?, ...} schema. Optionally pass `fallback` (typically the output of parse_assignment_email) so missing fields from the live fetch are backfilled from the email. AAN/manual platforms return manual_fetch_required:true with a dashboard URL.", inputSchema: { type: "object", properties: { platform: { type: "string", enum: ["filetrac", "xactanalysis", "aan", "manual"] }, claim_number: { type: "string" }, fallback: { type: "object", description: "Pre-known fields (e.g. parsed-email output). Used to backfill anything the live fetch doesn't return." }, refresh: { type: "boolean", description: "If false, skip live fetch and return fallback only. Default true." } }, required: ["platform", "claim_number"] } },
  { name: "scan_calendar_with_quadrants", description: "Read Google Calendar events over the next N days (default 3), geocode each event with a location, and tag with a freeway-corridor quadrant (Central/N/W/E/S relative to home). Returns events sorted by start time plus per-quadrant counts. Used by the post-claim slot picker to detect days where you'll already be in a given quadrant — those days are preferred for new claims in the same zone.", inputSchema: { type: "object", properties: { days: { type: "number", description: "Days to look ahead (default 3)" }, calendar_id: { type: "string", description: "Default 'primary'" }, time_min: { type: "string", description: "Override the start of the scan window (ISO datetime). Default: now." }, max_results: { type: "number", description: "Default 50" } }, required: [] } },
  { name: "create_claim_drive_folder", description: "D4 — per-claim Drive folder creator. Idempotent: reuses existing year/quarter/month folders; returns the existing claim folder if found (with `already_existed: true`) rather than duplicating. Builds the tree per locked 2026-05-04 convention: `Claims/{YYYY}/Q{n} (MMM–MMM)/{YYYY-MM}/{YYYY-MM-DD}_{(work-type) }Insured_Client_Carrier_LossType/Photos for Xactimate/`. **Date is when work was REQUESTED, not when Hakiel acts on it.** Pass `work_type` ('supplement' / 'reinspection' / 'reopen') for the parenthetical prefix; omit for original new assignments. Returns IDs + names + created/found status for every level + the human-readable path. **Notary signings get NO Drive folder** per convention — do not call this for notary work.", inputSchema: { type: "object", properties: { claims_root_id: { type: "string", description: "Override Claims/ root folder ID. Default: looks up name='Claims' folder at Drive root." }, request_date: { type: "string", description: "When work was REQUESTED (ISO datetime or YYYY-MM-DD). Drives placement and the YYYY-MM-DD prefix." }, insured_name: { type: "string", description: "Display name as it should appear in the folder, e.g. 'Raymond Rodriguez' or 'Kathleen Lowe Margarita Patino'." }, client_short: { type: "string", description: "Short client code: PCAS, SLG, USCS, etc." }, carrier_short: { type: "string", description: "Short carrier code: DBI, Fortegra, NARS, etc." }, loss_type: { type: "string", description: "Water / Wind / Vehicle / Fire / etc." }, work_type: { type: "string", enum: ["supplement", "reinspection", "reopen"], description: "Optional — adds the parenthetical prefix BEFORE the insured's name." } }, required: ["request_date", "insured_name", "client_short", "carrier_short", "loss_type"] } },
  { name: "check_followup_due", description: "Step 9 of the post-claim playbook (D3). Reads the Voice thread for an outbound SMS and checks whether the POC has replied since `sent_at`. If no reply AND we're past `threshold_hours` (default 3), fires an ntfy `[FOLLOWUP]` alert (priority 5, urgent) telling Hakiel to call the POC manually. **HARD RULE: NEVER auto-resends the SMS** — re-engagement must be a manual call from Hakiel. Single-shot tool; the caller (Cloud Dispatch or a cron) is responsible for 'fire once per claim' gating. Returns {ok, replied, reply?, hours_since_sent, threshold_hours, followup_fired, ntfy_url?}.", inputSchema: { type: "object", properties: { thread_id: { type: "string", description: "Voice thread id (with or without 't.' prefix)." }, insured_phone: { type: "string", description: "Used as `contact` lookup if thread_id not provided." }, sent_at: { type: "string", description: "ISO datetime — instant the outbound SMS was fired. Inbound messages with timestamp >= this count as a reply." }, insured_name: { type: "string", description: "Used in the alert title." }, threshold_hours: { type: "number", description: "How long to wait before firing the follow-up. Default 3." }, force_fire: { type: "boolean", description: "Fire the follow-up even if before threshold (test only)." }, ntfy_topic: { type: "string" }, ntfy_server: { type: "string" } }, required: ["sent_at", "insured_name"] } },
  { name: "handle_claim_approval", description: "Steps 8-10 of the post-claim playbook (D2) — called by Cloud Dispatch when Hakiel replies 'send' to the D1 approval prompt. Bundles three sequential operations: (1) voice_send_sms — fires the verbatim approved SMS to insured_phone; (2) calendar_create_event — adds [ADJ] event with color 6 (Tangerine, locked convention), title format '[ADJ] {Insured} Inspection — {Client or Carrier}', loss_address as location, and a description containing phone/claim/policy/examiner/special_instructions; (3) notion_create_database_item — logs the open-assignment row IF notion_database_id is provided. SMS failure aborts the pipeline (no calendar event for an unsent SMS). Calendar failure does NOT block Notion. Returns each step's status independently so callers can retry selectively. Pass skip_sms / skip_calendar / skip_notion to test individual stages.", inputSchema: { type: "object", properties: { insured_phone: { type: "string", description: "POC phone — accepts +1XXXXXXXXXX or 10-digit." }, sms_text: { type: "string", description: "Verbatim approved SMS body — typically from draft_inspection_sms." }, insured_name: { type: "string" }, carrier: { type: "string" }, client: { type: "string", description: "Short client code, e.g. 'Premier Claims'. Falls back to carrier if not set." }, claim_number: { type: "string", description: "Carrier's claim #." }, file_number: { type: "string", description: "FileTrac file # if different from claim_number." }, policy_number: { type: "string" }, date_of_loss: { type: "string" }, loss_type: { type: "string" }, examiner_name: { type: "string" }, examiner_email: { type: "string" }, examiner_phone: { type: "string" }, alternate_phone: { type: "string" }, loss_address: { type: "string" }, slot_start: { type: "string", description: "ISO datetime with offset, e.g. 2026-05-07T11:00:00-07:00." }, slot_end: { type: "string" }, special_instructions: { type: "string", description: "Optional notes from the assignment email — appended verbatim to the calendar description." }, event_title: { type: "string", description: "Optional — overrides the auto-generated title." }, calendar_id: { type: "string", description: "Default 'primary'." }, notion_database_id: { type: "string", description: "Optional — pass to write an open-assignment row." }, skip_sms: { type: "boolean" }, skip_calendar: { type: "boolean" }, skip_notion: { type: "boolean" }, voice_thread_id: { type: "string", description: "Optional — reuse an existing Voice thread instead of creating a new one." }, voice_skip_verify: { type: "boolean" } }, required: ["insured_phone", "sms_text", "insured_name", "carrier", "claim_number", "loss_address", "slot_start", "slot_end"] } },
  { name: "send_approval_prompt", description: "Step 7 of the post-claim playbook (D1) — push an approval prompt to Hakiel via ntfy on topic `dino-claims-alerts-fpx` with subject prefix `[APPROVE]`. Body includes the drafted SMS, claim # / file #, insured + carrier + phone + loss address, proposed slot with weekday + window + rationale, last-location anchor (the prev event location — REQUIRED so Hakiel can sanity-check routing), and prev/next drive-time slack. Hakiel replies through Cloud Dispatch ('send' fires the SMS, anything else holds). This tool only fires the prompt; D2 handles the approval → voice_send_sms wiring.", inputSchema: { type: "object", properties: { insured_name: { type: "string" }, carrier: { type: "string", description: "e.g. 'DB Insurance Company'" }, client: { type: "string", description: "Optional — the dispatching client (e.g. 'Premier Claims', 'Straight Line Global'). Shown as 'Carrier (Client)' in the prompt." }, claim_number: { type: "string", description: "Primary claim # (carrier's)" }, file_number: { type: "string", description: "Optional FileTrac file # if different from claim_number" }, claim_phone: { type: "string", description: "POC primary phone" }, loss_address: { type: "string" }, sms_text: { type: "string", description: "Verbatim rendered SMS — typically from draft_inspection_sms" }, slot: { type: "object", description: "Slot from pick_inspection_slots — minimal shape: { date, weekday?, start_label, end_label, rationale, feasible?, infeasible_reason?, prev_event_with_location?, next_event_with_location?, prev_leg?, next_leg? }" }, ntfy_topic: { type: "string", description: "Override topic (default: env CLAIM_MONITOR_NTFY_TOPIC or 'dino-claims-alerts-fpx')" }, ntfy_server: { type: "string", description: "Override server (default: env CLAIM_MONITOR_NTFY_SERVER or 'https://ntfy.sh')" }, reply_channel: { type: "string", description: "Where to reply for approval. Default 'Dispatch'." } }, required: ["insured_name", "carrier", "claim_number", "claim_phone", "sms_text", "slot"] } },
  { name: "draft_inspection_sms", description: "Render Hakiel's verbatim first-contact SMS template with the locked 2026-05-04 substitution rules. Inputs: slot_start + slot_end (ISO with offset, typically from pick_inspection_slots), and EITHER insured_first_names (string[]) OR insured_name (single string — will be split on /, &, ' and '). Output: sms_text plus the three computed substitution values for inspection. {first_name_or_names}: single → as-is, two → 'A and B', three+ → Oxford 'A, B, and C'. {proposed_date}: 'Tuesday May 5th' (day-of-week + full month + ordinal). {proposed_time_frame}: '7am-8am' on the hour, '9:30am-10:30am' otherwise (no spaces around dash, lowercase am/pm).", inputSchema: { type: "object", properties: { insured_first_names: { type: "array", items: { type: "string" }, description: "First names of insureds; preferred over insured_name." }, insured_name: { type: "string", description: "Combined insured_name field — will be auto-split on /, &, or ' and '." }, slot_start: { type: "string", description: "ISO datetime with offset, e.g. 2026-05-05T07:00:00-07:00" }, slot_end: { type: "string", description: "ISO datetime with offset" } }, required: ["slot_start", "slot_end"] } },
  { name: "pick_inspection_slots", description: "Steps 5+6 of the post-claim playbook (locked 2026-05-04). Given a loss address, geocode + classify its quadrant, scan the calendar up to `days` ahead (default 5, preferred 3), and return up to max_slots (default 5) candidate 1-hour inspection windows ranked by: (a) inside preferred_days first, (b) adjacency to a same-quadrant event, (c) earliest start. Working hours default 7AM–4PM LA. All-day events block their day. **Hard-block rule:** only events whose Google Calendar colorId is set AND not '9' (Blueberry) are inviolable conflicts ([ADJ]=6 Tangerine, [NOTARY]=3 Grape, etc.). Default-color and Blueberry events are soft and can be overlapped. **Today rule:** today only allowed if a same-quadrant timed event already exists today (geographically near); otherwise earliest = tomorrow 7AM. Each slot carries prev/next event-with-location anchors and (when validate_drive_times=true, default) prev_leg/next_leg drive data + feasible boolean. Set filter_infeasible=true to drop infeasible slots; set block_uncolored_events=true if Hakiel temporarily breaks color discipline.", inputSchema: { type: "object", properties: { loss_address: { type: "string" }, days: { type: "number", description: "Max days to scan (default 5)" }, preferred_days: { type: "number", description: "Slots inside this window rank first (default 3)" }, time_min: { type: "string", description: "Override 'now' (ISO). Default: now." }, calendar_id: { type: "string", description: "Default 'primary'" }, max_slots: { type: "number", description: "Default 5" }, work_start_hour: { type: "number", description: "LA-local working window start hour (default 7)" }, work_end_hour: { type: "number", description: "LA-local working window end hour (default 16)" }, slot_minutes: { type: "number", description: "Inspection window length in minutes (default 60)" }, validate_drive_times: { type: "boolean", description: "Run Distance Matrix on each slot's prev/next leg to set feasible flag. Default true." }, filter_infeasible: { type: "boolean", description: "Drop slots with feasible=false from the output. Default false (keep them visible with reason)." }, block_uncolored_events: { type: "boolean", description: "Treat every timed event as inviolable, ignoring colorId. Default false (rely on color discipline)." } }, required: ["loss_address"] } },
];
...
async function callTool(name: string, args: Record<string, unknown>) {
  switch (name) {
    ...
    case "gmail_email_to_pdf": return gmailEmailToPdf(args as any);
    ...
    case "notion_archive_page": return notionArchivePage(args as any);
    case "notion_restore_page": return notionRestorePage(args as any);
    case "notion_reorder_properties": return notionReorderProperties(args as any);
    ...
    case "xact_get_client_policy": return xactGetAssignment({ mfn: (args as any).mfn, tab: "client_policy" });
    case "xact_get_documents": return xactListDocuments(args as any);
    case "xact_download_document": return xactDownloadDocument(args as any);
    case "xact_get_assignment_notes": return xactGetAssignment({ mfn: (args as any).mfn, tab: "notes" });
    case "xact_get_history": return xactGetAssignment({ mfn: (args as any).mfn, tab: "history" });
    ...
    case "xact_delete_note": return xactDeleteNote(args as any);
    case "xact_set_planned_inspection_date": return xactSetPlannedInspectionDate(args as any);
    ...
    case "voice_list_threads": return voiceListThreads(args as any);
    case "voice_get_thread": return voiceGetThread(args as any);
    case "voice_dump_html": return voiceDumpHtml(args as any);
    case "voice_search_messages": return voiceSearchMessages(args as any);
    case "voice_get_voicemails": return voiceGetVoicemails(args as any);
    case "voice_send_sms": return voiceSendSms(args as any);
    ...
    case "notarygadget_update_customer": return notarygadgetUpdateCustomer(args as any);
    ...
    case "fetch_claim_details": return fetchClaimDetailsTool(args as any);
    case "scan_calendar_with_quadrants": return scanCalendarWithQuadrantsTool(args as any);
    case "pick_inspection_slots": return pickInspectionSlotsTool(args as any);
    case "draft_inspection_sms": return draftInspectionSmsTool(args as any);
    case "send_approval_prompt": return sendApprovalPromptTool(args as any);
    case "handle_claim_approval": return handleClaimApprovalTool(args as any);
    case "check_followup_due": return checkFollowupDueTool(args as any);
    case "create_claim_drive_folder": return createClaimDriveFolderTool(args as any);
