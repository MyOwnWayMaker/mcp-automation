...
const TOOLS: Tool[] = [
  ...
  { name: "voice_send_sms", description: "Google Voice send SMS with read-write-read verification (mirror of filetrac_add_note pattern). STUB — pending Dispatch's RPC probe.", inputSchema: { type: "object", properties: { thread_id: { type: "string" }, number: { type: "string" }, body: { type: "string" }, force: { type: "boolean", description: "Skip duplicate-suppression pre-check" }, skip_verify: { type: "boolean", description: "Skip post-write verification" } }, required: ["body"] } },
