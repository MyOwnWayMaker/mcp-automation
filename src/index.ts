...
const TOOLS: Tool[] = [
  ...
  { name: "xact_get_documents", description: "Returns the DOCUMENTS tab of an XA assignment — lists uploaded documents (typically dec pages, prior estimates). Convenience wrapper around xact_get_assignment with tab=documents.", inputSchema: { type: "object", properties: { mfn: { type: "string", description: "MFN code from the assignment URL (e.g. 06TSB9L)" } }, required: ["mfn"] } },
...
async function callTool(name: string, args: Record<string, unknown>) {
  switch (name) {
    ...
    case "xact_get_documents": return xactGetAssignment({ mfn: (args as any).mfn, tab: "documents" });
